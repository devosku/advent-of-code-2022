...
func processMonkeyRound(monkeys []monkey, inspections []int, lcm int) ([]monkey, []int) {
	newMonkeys := monkeys
	for i, monkey := range monkeys {
		for _, itemWorryLevel := range monkey.Items {
			inspections[i]++
			worryLevel := calcWorryLevel(monkey.Operation, itemWorryLevel)
...
func main() {
	...
	for i := 0; i < 10000; i++ {
		monkeys, inspections = processMonkeyRound(monkeys, inspections, lcm)
	}
